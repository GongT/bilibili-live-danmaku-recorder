...
def connect_message_queue(u, tls_ca):
    ...
    for i in QUEUE_NAMES:
        rmq.queue_declare(queue=queue_name(i), durable=True)
...
def url_parse(st):
    ...
    if u.username is None:
        raise argparse.ArgumentError(None, "URL中没有用户名")
    if u.password is None:
        raise argparse.ArgumentError(None, "URL中没有密码")
    if u.hostname is None:
        raise argparse.ArgumentError(None, "URL中没有服务器IP或域名")
    if u.port is None:
        raise argparse.ArgumentError(None, "URL中没有端口")
    return {"username": u.username, "password": u.password, "hostname": u.hostname, "port": u.port, "vhost": u.path}
